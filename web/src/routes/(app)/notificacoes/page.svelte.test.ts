...
describe('tela de notificações', () => {
	it('estado vazio quando não há nada', () => {
		const { getByText } = render(Page, { props: { form: null, data: data([], 0) } });
		expect(getByText('Nenhuma notificação')).toBeInTheDocument();
	});

	it('lista os títulos e o corpo', () => {
		const { getByText } = render(Page, {
			props: { form: null, data: data([notif({ title: 'Agendamento cancelado', body: 'foi cancelado' })], 1) }
		});
	...
	it('com não-lidas mostra o contador e o "marcar todas"', () => {
		const { getByText, getByRole } = render(Page, {
			props: { form: null, data: data([notif()], 3) }
		});
	...
	it('sem não-lidas mostra "Tudo em dia" e esconde o "marcar todas"', () => {
		const { getByText, queryByRole } = render(Page, {
			props: { form: null, data: data([notif({ read: true })], 0) }
		});
	...
	describe('abrir uma notificação', () => {
		it('marca lida, derruba o contador e SÓ ENTÃO navega para o destino', async () => {
			render(Page, { props: { form: null, data: data([notif({ kind: 'slot_opened' })], 1) } });
		...
		it('sem destino, só revalida', async () => {
			...
			render(Page, { props: { form: null, data: data([notif({ kind: 'daily_digest', data: {} })], 1) } });
	...
	describe('marcar uma como lida sem sair da tela', () => {
		it('linha não lida oferece o botão', () => {
			const { getByRole } = render(Page, {
				props: { form: null, data: data([notif({ title: 'Vaga aberta' })], 1) }
			});
		...
		it('linha já lida não oferece', () => {
			const { queryByRole } = render(Page, {
				props: { form: null, data: data([notif({ title: 'Vaga aberta', read: true })], 0) }
			});
	...
	describe('filtro', () => {
		...
		it('não desenha abas no corpo da página', () => {
			const { queryAllByRole, queryByRole } = render(Page, { props: { form: null, data: data([notif()], 3) } });
		...
		it('vazio com o filtro "não lidas" tem texto próprio', () => {
			const { getByText } = render(Page, {
				props: { form: null, data: data([], 0, { onlyUnread: true }) }
			});
	...
	describe('paginação', () => {
		it('caixa que cabe numa página não mostra o rodapé', () => {
			const { queryByRole } = render(Page, { props: { form: null, data: data([notif()], 1) } });
		...
		it('havendo mais, o rodapé aparece com "Anterior" desabilitado na primeira', () => {
			const { getByRole } = render(Page, {
				props: { form: null, data: data([notif()], 1, { more: true }) }
			});
		...
		it('na última página, "Próxima" desabilita e "Anterior" continua', () => {
			const { getByRole } = render(Page, {
				props: { form: null, data: data([notif()], 1, { more: false, current: 2 }) }
			});
...
// As três actions (`read`/`readAll`/`clearAll`) sempre responderam `fail(…, { action, error })`,
// mas a tela não declarava o `form`: a mensagem chegava e morria ali. "Limpar tudo" que falha
// fechava o diálogo, deixava a lista igual e não dizia uma palavra — indistinguível de uma caixa
// que já estava vazia.
describe('Notificações — erro da action', () => {
	it('erro do "limpar tudo" vira toast de ERRO com a mensagem do servidor', () => {
		render(Page, {
			props: {
				data: data([notif()], 1),
				form: { action: 'clearAll', error: 'Não foi possível concluir a operação.' }
			}
		});
		flushSync();

		expect(currentToast()?.message).toBe('Não foi possível concluir a operação.');
		expect(currentToast()?.variant).toBe('error');
	});

	it('falha sem mensagem cai na genérica — nunca em silêncio', () => {
		// `error: undefined` é a forma real do `fail` quando a API não mandou mensagem — o campo
		// existe, vazio. É esse caso que precisa cair na genérica.
		render(Page, {
			props: { data: data([notif()], 1), form: { action: 'read', error: undefined } }
		});
		flushSync();
	...
	it('sucesso não vira toast de erro', () => {
		render(Page, {
			props: { data: data([notif()], 1), form: { ok: true, action: 'readAll' } }
		});
		flushSync();
