...
// Busca de pacientes do `PatientPicker` (doc 25 §6). Endpoint próprio em vez de carregar o
// cadastro no load da agenda: paciente é a entidade que cresce sem limite, e a lista de
...
export const GET: RequestHandler = async (event) => {
	...
	// A regra dos 2 caracteres também vale no SERVIDOR: confiar no debounce do cliente
	// deixaria a porta aberta para um "a" varrer o cadastro a cada tecla.
	...
	// Projeção mínima: o picker desenha avatar, nome e telefone. CPF e o resto da ficha são
	// dado sensível e não têm por que trafegar para um autocomplete.
	return json({
