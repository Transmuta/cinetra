...
describe('POST /auth/sign-out-everywhere', () => {
	it('revoga na API (POST), apaga o cookie local e vai para /entrar', async () => {
		const { event, fetch, del } = fakeEvent();
		const r = await caught(() => POST(event));

		const [url, init] = fetch.mock.calls[0];
		expect(url).toContain('/api/auth/sign-out-everywhere');
