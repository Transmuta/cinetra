...
<section class="rounded-cartao border border-edge bg-surface p-5">
	<div class="mb-4 flex items-center gap-2.5">
		...
		<h2 class="flex-1 text-leitura font-bold">Próximas sessões</h2>
		{#if sessions.length}
	...
	<div class="overflow-hidden rounded-cartao border border-edge">
		{#if sessions.length === 0}
			...
			<div class="p-5 text-center text-corpo text-faint">Nenhuma sessão marcada.</div>
		{:else}
					...
					<li>
						...
						>
							...
							{#if i === 0}
								...
								<span
									class="shrink-0 rounded-full bg-accent-subtle px-2 py-0.5 text-meta font-semibold text-accent-text"
								>
									próxima
								</span>
							{/if}
