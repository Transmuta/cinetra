...
<section class="mt-3 rounded-controle border border-edge bg-surface p-4">
	<div class="mb-1 flex items-center gap-2">
		<ShieldCheck size={15} class="text-faint" />
		<h2 class="text-leitura font-semibold">O que cada papel pode</h2>
	</div>
	...
	<div class="overflow-x-auto">
		<table class="w-full min-w-[560px] border-collapse text-rotulo">
			<thead>
				...
				{#each matrix.areas as area (area.id)}
					<tr class="border-b border-edge/60 align-top">
						<th scope="row" class="py-2 pr-3 text-left font-medium text-ink">
							{area.label}
							{#if area.obs}
								<span class="mt-0.5 block text-meta font-normal text-faint">{area.obs}</span>
							{/if}
						</th>
						{#each matrix.papeis as papel (papel)}
							{@const meta = NIVEL_META[area.acesso[papel]]}
							<td class="px-2 py-2">
								<span class="inline-block rounded-micro px-1.5 py-0.5 text-meta font-semibold {TONE_CLASS[meta.tone]}">
									{meta.label}
								</span>
							</td>
						{/each}
