...
| F5 | ✅ **FEITO** (Onda 6, [doc 48 §3](48-onda-6-soltas-e-limpeza.md)) — só a visão de Dia rastreia; chave = usuário. | [`25 §9 (E3/E4)`](25-agenda.md) |
...
| F8 | **Eliminação de dados do titular (LGPD)** — resolução do **P4**. **ADIADO por decisão (2026-07-27), registrado como débito técnico em [`50 §D-1`](50-debitos-tecnicos.md): por ora o projeto fica só com soft-delete/arquivamento.** O bloqueio é **jurídico** (o prazo de guarda do COFFITO), não técnico. Fatia a construir, ancorada no **Paciente**. | Backend **não** iniciado. **Desenho decidido: hard delete** — `destroy` que apaga de verdade + `ON DELETE` podando a trilha junto (anonimização descartada). Atenção: contraria a política geral "sem hard delete" do projeto ([`25 §3`](25-agenda.md)) — é a exceção deliberada para o direito de eliminação; restringir por policy a owner/admin e provar sob RLS. | [`26 §5.1 (c)`](26-auditoria-bate-volta-agenda.md), P4 acima |
...
| V7 | ✅ **FEITO** (Onda 6, [doc 48 §5](48-onda-6-soltas-e-limpeza.md)) — motor construído do zero (não existia no backend), gate nas quatro portas, 409 com a lista. A precedência da RN-16 foi resolvida **apagando a regra**: a simulação passa pelo `Availability`. | [`25 §9 FORA`](25-agenda.md) |
...
| D-Aud1 | ~~**`COUNT(*)` do `countable: true` da tela de auditoria**~~ **ENCERRADO** pela retenção de 90 dias ([`63`](63-auditoria-completa.md), D-Aud5): a tabela passou a ser limitada por construção e o total voltou ao rodapé. Original: — conta a clínica inteira em todo request (inclusive página 1) na tabela que mais cresce. | **0,45 ms → 9,7 ms** de 2k → 52k linhas (~0,2 µs/linha → ~100 ms a 500k). Tela owner·admin de baixo tráfego. | Decisão de **produto**: total estimado (`reltuples`), `countable: false` + `limit+1`, ou contar só com filtro — todos trocam o rótulo exato "X–Y de Z". Decidir a semântica antes de mexer. | [`32 §5`](32-auditoria-bate-volta-tela-auditoria.md) |
| D-Aud2 | ~~**Filtro por autor (`user_id`) da auditoria faz Seq Scan da clínica**~~ **ENCERRADO**: com uma tabela só, o índice `(clinic_id, user_id, at DESC)` entrou junto ([`63`](63-auditoria-completa.md)) e o custo de escrita se dilui porque a retenção limita a tabela. Original: — sem índice cobrindo `user_id`. | **Seq Scan 50k linhas / 6 ms** (~60 ms a 500k). | A UI da v1 **não expõe** o filtro (só API/deep-link); índice `(clinic_id, user_id, version_inserted_at DESC)` adiciona custo de escrita na tabela mais escrita. Criar **quando a tela expuser** o filtro (serve filtro + ordenação de uma vez). `version_action_name` é da mesma classe, menos grave. | [`32 §5`](32-auditoria-bate-volta-tela-auditoria.md) |
...
| # | Dívida | Nota | Origem |
| --- | --- | --- | --- |
| D-M | **`mix test` não exercita RLS** — a suíte conecta como `postgres` (BYPASSRLS); bug de GUC ausente passa verde e só a tela em dev (ou prod) pega. | Correção: parte da suíte rodando como `cinetra_app` para o gate do CI pegar. Mexe no sandbox do Ecto — **decisão de infra**. | [`26 §5.1 (a)`](26-auditoria-bate-volta-agenda.md), [`25 §10`](25-agenda.md) |
...
| D-R | ✅ **MEDIDO E FECHADO** (Onda 6, [doc 48 §2d](48-onda-6-soltas-e-limpeza.md)) — 11 conexões com pool 10 (a 11ª é o `LISTEN` do Oban, fora do pool); as filas seguram até 7 das 10. `POOL_SIZE` → 16, com teste travando a aritmética. | [`26 §5.3 (i)`](26-auditoria-bate-volta-agenda.md) |
...
| D-U | ✅ **FEITO** (Onda 6, [doc 48 §2a/§2c](48-onda-6-soltas-e-limpeza.md)) — D2/D3/D7 consolidados; D4 e D5 já estavam feitos desde a Onda 5. | [`29 §5`](29-auditoria-bate-volta-fila-de-espera.md) |
